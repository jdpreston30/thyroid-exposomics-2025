...
\textbf{Online Code and Data Repository}

All source code is available in the accompanying GitHub repository, accessible at the following URL: https://github.com/jdpreston30/thyroid-exposomics-2025. All elements of the repository are listed below, along with a description and guide of each:

\begin{itemize}
\item \textbf{source\_code.R:} This file is the main analysis script for the project. All data required to run the code, excluding data required for the blocks under ``\#+ Demographics (Table 1)'', can be imported from the ``Data and Metadata Files'' folder or loaded into the global environment from the compiled R serialized object (all\_objects.rds). The following packages are required for the source code: broom, dplyr, forcats, ggplot2, multcompView, purrr, readxl, stringr, tibble, tidyr, webchem, writexl. These can all be installed and loaded under the header ``\#* Dependencies.'' A version of R $\geq$ 4·5.1 is recommended for running all analyses in the source code.

\item \textbf{all\_objects.rds:} This is the R serialized object containing all objects and the full dataset, excluding demographic data.

\item \textbf{Data and Metadata Files:} This folder contains all primary data and metadata files. These data can be directly imported into the analysis script. A description of each file is as follows:

\begin{itemize}
\item \textbf{primary\_data.xlsx:} This file contains the primary data for this project. A description of each tab in the file is as follows:
\begin{itemize}
\item lib.subject.summary: Feature table for all annotated features.
\item tumors\_sequence: Sequence file and associated sample metadata.
\item lib.subject.qusummary: Quantitative feature table for tumors.
\item lib.subject.qsummary.cadaver: Quantitative feature table for non-cancer cadaver thyroids.
\item library: Information on the full library of standards used.
\item tissue\_weights: Weights of each tissue sample.
\end{itemize}

\item \textbf{chemical\_metadata.xlsx:} Metadata for all detected chemicals.

\item \textbf{feature\_metadata:} Detailed metadata for all unique identifications.

\item \textbf{endogenous\_excluded\_features:} Features excluded from analysis due to being primarily endogenous.

\item \textbf{supplementary\_tables\_raw.xlsx:} Raw forms of the tables used in the supplementary material.
\begin{itemize}
\item ST1\_raw: raw form of Supplementary Table 1
\item ST2\_raw: raw form of Supplementary Table 2
\item ST3\_raw: raw form of Supplementary Table 3
\item ST4\_raw: raw form of Supplementary Table 4
\item Abbreviation legend for the Supplementary Tables
\end{itemize}
\end{itemize}

\item \textbf{Spectral Validation:} This is the directory for spectral validation performed on select identifications.

\item \textbf{Outputs:} This folder contains all output data files generated by running the source code.

\item \textbf{Figures.Prism:} This is a GraphPad Prism file used to create all figures in the manuscript.
\end{itemize}

\newpage

\textbf{Exposome-Wide Association Study}
...
\textbf{Advanced Carcinogenicity Classification of Select Chemicals}
...
\item A chemical concentration of 0 ng/mL results in an intensity of 0 (i.e., the origin of (0,0)). This is used in combination with the intensity from the known concentration in the SRM (0·5 ng/mL) to establish a two-point standard curve, which assumes linearity.
\item While each chemical has a concentration in the SRM of 0·5 ng/mL, the concentration in 50 $\mu$L of isooctane is 2 ng/mL. This is because 200 $\mu$L of SRM plasma is used, and therefore, 0·1 ng of the total EC of interest is extracted. This is dried after extraction and reconstituted in 50 $\mu$L of isooctane, resulting in a concentration of 2 ng/mL in the injected solution.
\end{itemize}
...
$$y = \beta x$$
where $y$ is the known concentration of the EC in the SRM (0·5 ng/mL for all ECs analyzed), $x$ is the observed spectral intensity for the given EC, and $\beta$ is the slope of the calibration curve, defined by the line through the origin (0,0) and the point $(x,y)$.
...
$$C_e = \beta I$$
where $I$ is the observed spectral intensity for the given EC in the sample. However, to determine the true concentration in the isooctane solvent ($C_{es}$), we must scale $C_e$ by a factor of 2 to account for the concentration that occurs during extraction. Specifically, the 200 $\mu$L of plasma standard extracted contains 0·1 ng total of the EC (0·5 ng/mL). Assuming 100\% extraction, this 0·1 ng was then resuspended in 50 $\mu$L isooctane (2 ng/mL), Therefore, we use a 2$\times$ scaling factor:
$$C_{es} = 2C_e$$
...
Alternatively, algorithm outputs are used directly without unit conversions, the following formula can be used, provided $C_e$ is in ng/mL and $M_T$ is in mg:
$$\text{PPM} = \frac{C_e \times 10^2}{M_T}$$
$$\text{PPB} = \frac{C_e \times 10^5}{M_T}$$
\end{enumerate}

\textbf{Quantitative Comparisons of Tumors and Non-Cancer Cadaver Thyroids}
...
\textbf{Comparing Observed Concentrations to Literature Values for Select Chemicals}
...
\begin{center}
\textbf{\large TABLE ABBREVIATION DICTIONARY}
\end{center}
