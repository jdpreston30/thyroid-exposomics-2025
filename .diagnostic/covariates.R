#* 1: Covariate Confounding Investigation (reviewer comment #2)
...
#+ 1.1: Recover + VERIFY per-row sample metadata (sample_ID, variant, year, Age, Sex)
stopifnot(exists("tumor_column"), exists("tumor_seq"), exists("tumors_quant_wt"),
...
stopifnot(identical(.vseq, as.character(tumors_quant_wt$variant)),  # interleaved 60/60 => aligned
...
cat(sprintf("[setup] %d samples aligned & verified | year %d-%d | quant feats=%d qual feats=%d\n",
            nrow(sample_meta), min(sample_meta$year), max(sample_meta$year),
            ncol(tumors_quant_wt) - 1L, ncol(tumors_qual) - 1L))

#+ 1.2: Helper — significant-vs-chance summary for a vector of p-values
.pchance <- function(p, label) {
  p <- p[!is.na(p)]; n <- length(p); obs <- sum(p < 0.05); exp <- 0.05 * n
  cat(sprintf("   %-20s %3d/%4d sig (%.1f%%) vs ~%.1f by chance  [%s]\n",
              label, obs, n, 100 * obs / n, exp,
              if (obs <= exp * 1.5) "~chance"
              else if (obs >= exp * 2) "ABOVE chance" else "modest excess"))
  invisible(c(obs = obs, n = n, exp = exp))
}

#* 2: Covariate balance by tumor type (does each covariate differ by type?)
cat("\n#* 2: COVARIATE BALANCE BY TYPE ------------------------------------------\n")
#+ 2.1: Age
cat(sprintf("Age:  ANOVA p=%.3f | Kruskal-Wallis p=%.3f\n",
...
#+ 2.3: Collection year — binned (paper-style) vs continuous (the key reconciliation)
.bins <- cut(sample_meta$year, breaks = c(2005, 2009, 2013, 2017, 2021),
             labels = c("2006-2009", "2010-2013", "2014-2017", "2018-2021"))
cat(sprintf("Year: binned Fisher p=%.4f  |  continuous KW p=%.4f | ANOVA p=%.4f\n",
...
#* 3: Covariate <-> chemical screens (all features), with chance baselines
cat("\n#* 3: COVARIATE <-> CHEMICAL SCREENS (observed sig vs chance) --------------\n")
#+ 3.1: Quant features (Spearman for year/age; Wilcoxon for sex)
.qcols <- setdiff(names(tumors_quant_wt), "variant")
.screen_q <- function(cov, binary = FALSE) vapply(.qcols, function(cn) {
  y <- tumors_quant_wt[[cn]]
  tryCatch(if (binary) wilcox.test(y ~ cov)$p.value
           else suppressWarnings(cor.test(y, cov, method = "spearman")$p.value),
...
#+ 3.2: Qual features (detection 0/1): covariate ~ detection (testable features only)
.dcols <- setdiff(names(tumors_qual), "variant")
.testable <- .dcols[vapply(.dcols, function(cn) {
  v <- tumors_qual[[cn]]; length(unique(v[!is.na(v)])) > 1 }, logical(1))]
.screen_ql <- function(cov, cat = FALSE) vapply(.testable, function(cn) {
...
#* 4: Targeted ANCOVA — do type effects survive adjustment for year (+age+sex)?
...
.anc_ql <- function(cn) {
  ...
  lrt <- function(a, b) if (!is.null(a) && !is.null(b))
    tryCatch(anova(a, b, test = "LRT")[["Pr(>Chi)"]][2], error = function(e) NA_real_) else NA_real_
  data.frame(name_sub_lib_id = cn, mode = "qual",
...
ancova_res <- rbind(do.call(rbind, lapply(.valq, .anc_q)),
...
print(within(ancova_res, {
  p_unadj <- signif(p_unadj, 3); p_yearAdj <- signif(p_yearAdj, 3); p_fullAdj <- signif(p_fullAdj, 3)
})[, c("short_name", "mode", "p_unadj", "p_yearAdj", "p_fullAdj", "survives_year", "flips")], row.names = FALSE)

#* 5: Common support — collection-year overlap for each type pair
...
for (pr in combn(names(.yr), 2, simplify = FALSE)) {
  ...
  cat(sprintf("  %-11s vs %-11s overlap %s | n_%s=%d, n_%s=%d in window\n",
              pr[1], pr[2], if (lo <= hi) sprintf("%d-%d", lo, hi) else "NONE",
              pr[1], sum(a >= lo & a <= hi), pr[2], sum(b >= lo & b <= hi)))
}

#* 6: Summary for the reviewer response
cat("\n#* 6: SUMMARY ------------------------------------------------------------\n")
...
cat(sprintf("ANCOVA: of %d originally-significant validated chemicals, %d lose significance after year adj.\n",
            .nsig, .nflip))
cat(sprintf("=> %s\n", if (.nflip == 0) "ALL type effects survive year adjustment — robust."
           else sprintf("%d effect(s) year-sensitive — flag those; the rest are robust.", .nflip)))
