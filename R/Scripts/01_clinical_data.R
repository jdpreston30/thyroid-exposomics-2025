...
cadaver_metadata <- cadaver_metadata_raw |>
  filter(seq != "T-006") |> #! T-006 was HBcAb reactive, destroyed, and never run; seq is "T-00N" so the previous bare "006" silently matched nothing and left 9 rows
  mutate(collection_year = as.integer(format(DOD, "%Y"))) |> # extract collection year from DOD
  select(seq, age, sex, collection_year)
stopifnot(nrow(cadaver_metadata) == 8) # guard: the filter above failed silently once
