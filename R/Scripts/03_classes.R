...
classes <- feature_metadata |>
  ...
  mutate(Class = str_to_title(Class),
         Class = str_replace_all(Class, "\\b(And|Or|Of|The|A)\\b", tolower)) |>
  arrange(Class == "Other", desc(n), Class) |>
  rename(Graph_Class = Class)
#- 3.3.3: Unique class counts printout
cat(
  "Total chemicals:", nrow(feature_metadata), "\n",
  "Unique superclasses:", n_distinct(feature_metadata$Superclass, na.rm = TRUE), "\n",
