...
adjust_VP_colors <- function(validation_plots) {
  ...
  for (i in seq_along(validation_plots)) {
    ...
    if (!is.null(original_plot$data) && "mz_label" %in% names(original_plot$data)) {
      ...
      #- Sort by mz number (extract numeric part for sorting)
      mz_nums <- as.numeric(gsub("^mz([0-9]+):.*", "\\1", present_mz))
      present_mz <- present_mz[order(mz_nums)]
      ...
      #- Apply color scale (use "color" to match American spelling in aes())
