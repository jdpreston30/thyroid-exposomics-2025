...
#! Device writes to tempdir and the finished PDF is copied in: the repo sits in iCloud's Desktop sync, which has swapped a file's inode under an open multi-page device (compile_validation_pdf(), script 14; punch list 133).
  tmp_path <- tempfile("sf_sub_", fileext = ".pdf")
  cairo_pdf(tmp_path, width = 8.5, height = 11, onefile = TRUE, family = "Arial")
  walk(pages, print)
  ...
  if (!file.copy(tmp_path, output_path, overwrite = TRUE)) stop("compile_sf_sub_pdf: could not copy ", basename(output_path), " into ", output_dir)
  unlink(tmp_path)
