...
#' @export
zoom_fragment <- function(plot_obj, mz_fragment, subfolder = "revised") {
  # Validate input
  ...
  target_label <- paste0("mz", mz_num)
  # Check for asterisk variant
  target_with_asterisk <- paste0(target_label, "*")
  
  if (target_with_asterisk %in% available_fragments) {
    target_label <- target_with_asterisk
  } else if (!target_label %in% available_fragments) {
  ...
  if (mz_fragment > 0) {
    ...
    plot_data_filtered <- plot_data[plot_data$mz_label != target_label, ]
    subtitle_text <- sprintf("Removed: %s", target_label)
  }
  ...
  # Update plot subtitle to indicate fragment modification
  modified_plot$plot <- modified_plot$plot +
    ggplot2::labs(
