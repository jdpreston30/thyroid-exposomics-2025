#* Dependencies and Full Data Import
  #+ Dependencies
...
#* Preprocessing
  #+ Import raw data and sequence data
    tumor_raw <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.summary")
    tumor_seq <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "tumors_sequence") %>%
      select(ID,variant) %>%
      unique()
  #+ Structure data
  ...
  #+ Split into quant and qual, 1/2 NA replace
    na_threshold <- 0.3
    PMV <- colMeans(is.na(tumor)) # calculate proportion of missing values for each feature
    cols_quant <- names(PMV[PMV <= na_threshold]) # Keep
    cols_qual <- names(PMV[PMV > na_threshold]) # Remove
    cols_quant <- unique(c("variant", cols_quant))
    cols_qual <- setdiff(cols_qual, "variant") # Ensure 'variant' is not removed
    tumors_quant <- tumor %>%
...
#* Analysis
  #+ Demographics (Table 1)
    #- Import data
      demographics <- read_excel("NFS/Manifest.xlsx", sheet = "Manifest") %>%
    ...
    #- Combine
      ...
      write.csv(demographic_table,"demographics.csv")
  #+ Detection (Figure 1A and B)
    #- Group by CAS and get total unique which were detected
      detection <- tumor_raw %>%
    ...
    #- Import the list which is purely endogenous which was compiled externally from above
      endog <- read_excel("Supplementary Data Files/Supplementary File 3.xlsx", sheet = "Endogenous Excluded Features") %>%
        select(cas)
      endog_cas <- endog$cas
    #- Remove those from the detection list
      detection_no_endog <- detection %>%
        select(-any_of(endog_cas))
      detection_no_endog <- detection_no_endog %>%
        mutate(total_detected = rowSums(across(-variant))) %>%
        select(variant,total_detected) %>%
        arrange(total_detected) %>%
        arrange(variant)
      write.csv(detection_no_endog, "detection_no_endog.csv")
    #- Now, make a frequency distribution with bins for Prism
      ...
      write.csv(freq_dist_bins, "freq_dist_bins.csv")
    #- Running Kruskal-Wallis Test since we are comparing medians and using count data, even though data are normally distributed
      kruskal.test(total_detected ~ variant, data = detection_no_endog)
      #! Graphed in prism at this point
  #+ Use Classes (Figure 2A)
    feature_metadata <- read_excel("Supplementary Data Files/Supplementary File 3.xlsx", sheet = "feature_metadata")
  ...
  #+ ANOVA and Fisher's (3A and 3B)
    #- ANOVA Stats
      #_Import the weights of the tissues
        weights <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "tissue_weights") %>%
    ...
    #- Compiled ANOVA and Fisher's, add short name, export qual for Prism
      #_Add updated quant names to the excel
        short_name <- read_excel("Supplementary Data Files/Supplementary File 3.xlsx", sheet = "feature_metadata") %>%
  ...
  #+ Determine est PPM/PPB for detected samples and controls, join (SF4)
    #- Import the quantified data
      ...
      #_Import raw concentration data
        conc_raw <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.qsummary",col_type = "text") %>%
          select(name_sub_lib_id,F1:F20) %>%
    ...
    #- Determine PPM and PPB for controls
      #_Import control tissue weights
        cadaver_tissue_wts <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "tissue_weights") %>%
          filter(samples == "Control") %>%
          select(ID, weight_mg) %>%
          rename(control_ID = ID)
      #_Import control conc data, normalize by tissue weight, compute PPM/PPB
        cadaver_qraw <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.qsummary.cadaver") %>%
  ...
  #+ All Inner join (only retain features in TUMORS that match controls) (SF4)
    ...
    #- Bring in appropriate metadata to make prioritization decisions
      fragment_quality_info <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.qsummary") %>%
        arrange(cas) %>%
  ...
  #+ Create subsetted table from master (Table 3)
    ...
    #- Create table columns and format
      table_3 <- MT_final %>%
        ...
        select(short_name, Table_Class, FTC_let:PTC_let, p_value) %>%
        arrange(p_value)
    #- Export
  ...
  #+ Pull IARC 1 features (3D)
    ...
    #- Now pull the IARC1s in controls and tumors
      IARC_controls <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.qsummary.cadaver") %>%
        filter(!is.na(comp)) %>%
        filter(pct_NA <= 0.5) %>%
        filter(cas %in% iarc_1) %>%
        select(pct_NA,name_sub_lib_id,iMean) %>%
        rename(pct_NA_ctrl = pct_NA, iMean_ctrl = iMean)
      IARC_tumors <- read_excel("Supplementary Data Files/Supplementary File 2.xlsx", sheet = "lib.subject.qsummary") %>%
        filter(cas %in% iarc_1) %>%
        filter(pct_NA <= 0.7) %>%
